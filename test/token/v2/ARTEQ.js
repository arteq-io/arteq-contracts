...
describe('ARTEQ', function() {
  ...
  beforeEach(async () => {
    ...
    const TaskManagerContract = await hre.ethers.getContractFactory("arteQTaskManager", deployer);
    ...
    const ARTEQContract = await hre.ethers.getContractFactory("contracts/token/v2/ARTEQ.sol:ARTEQ", deployer);
    await expect(ARTEQContract.deploy(ZeroAddress))
      .to.be.revertedWith("ARTEQ: zero address set for task manager");
    contract = await ARTEQContract.deploy(tmContract.address);
    ...
    expect(await contract.connect(trader1).name()).to.equal("arteQ NFT Investment Fund");
    expect(await contract.connect(trader1).symbol()).to.equal("ARTEQ");
